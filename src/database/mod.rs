...
use structs::{
    CompressionAlgorithm, DatabaseFile, EncryptionAlgorithm, Header, HeaderFields, InternalContent,
    KDFParams, KeyDerivationFunction, SaveMethod,
};
...
impl AppState {
    ...
    pub fn is_db_open(&self) -> Result<bool, String> {
        let guard = self
            .current_db
            .lock()
    ...
    ///
    /// Note: The caller should NOT expect the database file contents to be returned.
    /// Instead, subsequent calls should operate against the in-memory database.
    pub fn load_db(&self, path: &str, master_password: &str) -> Result<(), String> {
    ...
    pub fn save_db(&self, path: &str, master_password: &str, atomic: bool) -> Result<(), String> {
        let mut guard = self
            .current_db
            .lock()
            .map_err(|_| "state poisoned".to_string())?;
